...
class SUBMINLIN(object):
    ...
    def EAMC(self, B):
        ...
        kn = int(self.n*self.n)
        print('start While, print every kn=', kn, ' iters')
        time0 = time.time()
        while t < T:
            if iter1 == kn:
                iter1 = 0
                ...
                for p in range(0, self.n+1):
                    if Yfitness[p, 1] <= B and Yfitness[p, 0] > maxValue:
                        ...
                        resultIndex = p
                print('t=', t, ' f(x), cost(x), |x|, g(x) = ', Yfitness[resultIndex, :], popSize)
            ...
            for i in range(1,self.n+1):
                if Xfitness[i,2]>0:
                    ...
                    if Yfitness[i,2]>0:
                        tempSize=tempSize+1
                if Zfitness[i, 2] > 0:
                    if Wfitness[i, 2] > 0 and zwsame[i]==1:#np.linalg.norm(Z[i, :] - W[i, :]) == 0:  # same
                        tempSize = tempSize + 1
                    if Wfitness[i, 2] > 0 and zwsame[i]==0:# np.linalg.norm(Z[i, :] - W[i, :]) > 0:
                        tempSize = tempSize + 2
                    if Wfitness[i, 2] == 0:
                        tempSize = tempSize + 1
                else:
            ...
            for i in range(1,self.n+1):
                if Xfitness[i, 2] > 0:
                    ...
                    if Yfitness[i, 2] > 0:
                        ...
                        j = j + 1
                if Zfitness[i, 2] > 0:
                    if Wfitness[i, 2] > 0 and zwsame[i] == 1:
                    #if Wfitness[i, 2] > 0 and np.linalg.norm(Z[i, :] - W[i, :]) == 0:  # same
                        population[j, :] = Z[i, :]
                        j = j + 1
                    if Wfitness[i, 2] > 0 and zwsame[i] == 0:
                    #if Wfitness[i, 2] > 0 and np.linalg.norm(Z[i, :] - W[i, :]) > 0:
                        population[j, :] = Z[i, :]
                        j = j + 1
                        population[j, :] = W[i, :]
                        j = j + 1
